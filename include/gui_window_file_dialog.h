...
void GuiWindowFileDialog(GuiWindowFileDialogState *state)
{
    ...
    {
        bool wasGuiLocked = GuiIsLocked();
        if (state->overwriteDialogOpened) GuiLock();
        // Update window dragging
        //----------------------------------------------------------------------------------------
        if (state->supportDrag && !state->overwriteDialogOpened)
        {
        ...
        //----------------------------------------------------------------------------------------
        state->windowActive = !GuiWindowBox(state->windowBounds, state->saveFileMode ? "#198# Save File" : "#198# Open File");
...
# endif
        ...
        {
            if (!state->saveFileMode || !IsPathFile(TextFormat("%s" PATH_SEPERATOR "%s", state->dirPathText, state->fileNameText))) state->windowActive = false;
            else state->overwriteDialogOpened = true;
        }

        // Saving file to already existing file
        if (state->overwriteDialogOpened)
        {
            GuiUnlock();
            int result = GuiMessageBox((Rectangle){ GetScreenWidth() / 2 - 125, GetScreenHeight() / 2 - 50, 250, 100 },
                    "#191#Warning", "This file already exists", "Cancel;Overwrite");
            GuiLock();
            if (result == 0 || result == 1)
            {
                state->overwriteDialogOpened = false;
                if (!wasGuiLocked) GuiUnlock();
            }
            else if (result == 2)
            {
                state->windowActive = false;
